...
<body class="bg-white">
	...
	<main>
		<div class="container-lg py-4">
			...
			<!-- 날짜 네비게이션 -->
			<div
				class="content-box d-flex justify-content-between align-items-center mb-4">
				<!-- 왼쪽: 제목 -->
				<div class="section-title">오늘 일정</div>

				<div class="d-flex align-items-center gap-2">
					<!-- 이전 날 버튼 -->
					<a href="?trainerId=${param.trainerId}&date=<%= prevDate %>"
						class="btn btn-sm btn-outline-secondary px-2">&lt;</a>

					<!-- 날짜 텍스트 & 숨겨진 input[type=date] -->
					<form method="get" class="m-0 p-0 d-flex align-items-center">
						...
						<label class="date-display"
							...
							style="cursor: pointer;"> <%=formattedDate%>
						</label>

						<!-- 숨겨진 input[type=date] -->
						<input type="date" name="date" id="datePicker"
							class="form-control form-control-sm visually-hidden"
							value="<%=date.toString()%>" onchange="this.form.submit()" />

					</form>

					<!-- 다음 날 버튼 -->
					<a href="?trainerId=${param.trainerId}&date=<%= nextDate %>"
						class="btn btn-sm btn-outline-secondary px-2">&gt;</a>
				</div>
			</div>
			...
			<div class="content-box">
				<c:forEach var="pt" items="${ptScheduleList}">
					<c:if
						test="${pt.trainerId == param.trainerId && pt.bookDate == compareDate}">
			...
			<%-- 				<c:forEach var="q" items="${pageList}"> --%>
